...
export default function App() {
  ...
  function handleSplitBill(val) {
    ...
    setFriends((curr) =>
      curr.map((obj) =>
        selectedFriend.id === obj.id
          ? { ...obj, balance: val + obj.balance }
          : obj
      )
    );
...
function FormSplitBill({ currSelected, onSplitBill }) {
  ...
  return (
    <form className="form-split-bill" onSubmit={handleSubmit}>
      <h2>Split a bill with {currSelected.name}</h2>
      ...
      <label>🛑{currSelected.name}'s expenses</label>
      ...
      >
        ...
        <option value="friend">{currSelected.name}</option>
      </select>
